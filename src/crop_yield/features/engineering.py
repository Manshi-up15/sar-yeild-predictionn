...
class FeatureExtractor:
    ...
    def extract_texture(self, image_path: Path) -> Dict[str, float]:
        ...
        # Quantize the float backscatters into 8 gray levels (0 to 7)
        b_min, b_max = band.min(), band.max()
        if b_max - b_min < 1e-5:
            img_quant = np.zeros_like(band, dtype=np.int32)
        else:
            img_quant = np.digitize(band, np.linspace(b_min, b_max, 8)) - 1
    ...
    def extract_temporal_metrics(self, time_series_paths: List[Path]) -> Dict[str, float]:
        """
        Computes multi-temporal aggregate metrics (min, max, mean, and trend slope)
        across a series of processed SAR image paths.
        """
        logger.info(f"Extracting temporal statistics across {len(time_series_paths)} scenes.")
    ...
    def run_pipeline(
        ...
        """
        logger.info(f"Running feature extraction pipeline. Image: {preprocessed_image.name}, Weather: {weather_data.name}")
        ...
        sar_factor = 0.1 * (features["vv_mean"] + 15.0)  # less negative dB = higher biomass
        
        simulated_yield = base_yield + temp_factor + rain_factor + soil_factor + sar_factor
