...
# App Header
st.markdown(
    '<div class="main-title">🌾 AI Crop Yield Prediction System</div>',
    unsafe_allow_html=True,
)
st.markdown("""
...
if task == "📊 Dashboard Overview":
    ...
    st.markdown(
        '<div class="section-header">📜 Historical Predictions Log</div>',
        unsafe_allow_html=True,
    )
    ...
    if features_csv.exists():
        try:
            ...
            if "target_yield" in df_hist.columns and len(df_hist) > 0:
                ...
                fig_hist.update_layout(
                    yaxis_title="Estimated Yield (tons/ha)", xaxis_title="Record Index"
                )
        ...
        st.info(
            "No prediction history found. Go to 'Data Ingestion' or 'Model Training' to build your dataset."
        )

elif task == "📥 Data Ingestion":
    st.markdown(
        '<div class="section-header">📥 Sentinel-1 & Weather Data Ingestion</div>',
        unsafe_allow_html=True,
    )
    ...
    with col_l:
        st.write(
            "Configure details for area of interest and timeframe to trigger downloads."
        )
        ...
        if st.button("Start Ingestion Pipeline"):
            with st.spinner(
                "Ingesting Sentinel-1 satellite grids and daily weather metrics..."
            ):
                import json

                try:
                    from crop_yield.data.ingestion import (
                        Sentinel1Downloader,
                        WeatherDataIngester,
                    )
                    ...
                    st.success(
                        ...
                        VV Band:
                        {Path(raw_tif["vv_path"]).name}

                        VH Band:
                        {Path(raw_tif["vh_path"]).name}
    ...
    with col_r:
        ...
        if weather_csv.exists():
            ...
            st.info(
                "No weather data compiled yet. Run ingestion to view meteorological trends."
            )

elif task == "⚙️ Run Preprocessing":
    st.markdown(
        '<div class="section-header">⚙️ Radiometric Calibration & Speckle Filtering</div>',
        unsafe_allow_html=True,
    )
    st.write(
        "Convert raw radar intensity grids to cleaned, orthorectified and dB normalized backscatter bands."
    )

    from pathlib import Path

    safe_root = settings.RAW_DATA_DIR / "sentinel1"

    vv_files = list(safe_root.glob("*.SAFE/measurement/*vv*.tif*"))

    if not vv_files:
        st.warning("No Sentinel-1 VV GeoTIFF found. Trigger Data Ingestion first.")
    else:
        raw_path = vv_files[0]

        st.success(f"Detected VV raster:\n{raw_path.name}")
    ...
    if st.button("Execute Preprocessing Pipeline"):
        with st.spinner(
            "Executing calibration, Lee Filtering, reprojection, and db normalization..."
        ):
            try:
    ...
    if processed_files:
        ...
        try:
            with rasterio.open(raw_path) as r_src:
                raw_vv = r_src.read(1, out_shape=(256, 256)).astype("float32")

            with rasterio.open(processed_tif) as p_src:
                proc_vv = p_src.read(1, out_shape=(256, 256)).astype("float32")
    ...
    st.markdown(
        '<div class="section-header">🏋️ Train Crop Yield Forecasting Models</div>',
        unsafe_allow_html=True,
    )
    ...
    if not processed_files or not weather_files:
        ...
        if st.button("Extract Features"):
            with st.spinner(
                "Extracting spatial statistics, GLCM texture, temporal, and weather features..."
            ):
                try:
                    from crop_yield.features.engineering import FeatureExtractor

                    extractor = FeatureExtractor()
                    feat_csv = extractor.run_pipeline(
                        processed_files[0], weather_files[0]
                    )
                    st.success(
                        f"Feature engineering dataset updated at: {feat_csv.name}"
                    )
                except Exception as e:
    ...
    model_type = st.selectbox(
        ...
        [
            ...
            "XGBoost",
            "Linear Regression",
            "CropCNN (Deep Learning)",
            "CropCNNLSTM (Deep Learning)",
        ],
    )
            ...
            with st.spinner(f"Fitting model: {model_type}..."):
                try:
                    ...
                    if m_type_clean in [
                        ...
                        trainer.save_model()
                        st.success(
                            f"Model fitted successfully! RMSE: {summary['metrics']['rmse']:.4f}, R²: {summary['metrics']['r2']:.4f}"
                        )
                        ...
                        fig_comp.update_layout(
                            title="Model Validation Metrics Summary",
                            template="plotly_dark",
                        )
                    ...
                    # 2. Deep Learning PyTorch
                        ...
                        from crop_yield.models.deep_learning import (
                            SARDataset,
                        ...
                        loader = DataLoader(
                            dataset, batch_size=min(4, len(dataset)), shuffle=True
                        )

                        name_mapping = {"cropcnn": "cnn", "cropcnnlstm": "cnn_lstm"}
                        trainer_dl = DeepLearningModelTrainer(
                            model_name=name_mapping[m_type_clean]
                        )
                        history = trainer_dl.train_model(
                            train_loader=loader, val_loader=loader, epochs=epochs_val
                        )
    ...
    st.markdown(
        '<div class="section-header">🔮 Real-time Yield Prediction Inference</div>',
        unsafe_allow_html=True,
    )
    st.write(
        "Adjust agricultural feature inputs to forecast overall crop yield output."
    )
    ...
    with col_f1:
        ...
        vv_mean = st.slider(
            "VV Backscatter Mean (dB)",
            -25.0,
            -5.0,
            -12.5,
            help="Double-bounce vegetative radar reflectivity",
        )
        vh_mean = st.slider(
            "VH Backscatter Mean (dB)",
            -30.0,
            -10.0,
            -18.2,
            help="Cross-polarization volume scattering",
        )
    ...
    with col_f2:
        ...
        rainfall_val = st.number_input(
            "Total Seasonal Rainfall (mm)", min_value=0.0, max_value=2000.0, value=320.0
        )
        soil_m = st.slider("Mean Soil Moisture Fraction", 0.05, 0.65, 0.32)

        st.markdown("#### 🧠 Model Selection")
        pred_model_type = st.selectbox(
            "Prediction Engine", ["Random Forest", "XGBoost", "Linear Regression"]
        )
        ...
        if st.button("Forecast Estimated Yield"):
            ...
            try:
                payload = {"features": features_dict, "model_type": m_type_clean}

                response = requests.post(
                    "http://127.0.0.1:8000/predict", json=payload, timeout=30
                )
